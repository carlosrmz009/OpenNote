...
pub enum Target {
    ...
    Fingers,
    /// Which finger plays each note, graded against the fingerings that are taught.
    ///
    /// The material is generated rather than collected: scales, major and chromatic, in
    /// a random key, hand, register, span, direction and speed. Their fingerings have a
    /// settled answer, which is what makes them exercises in the first place, so the
    /// labels survive being generated in a way that labels cut out of real music do not
    /// — a fingering in a piece is right *for the passage around it*, and shuffling
    /// passages keeps the label while throwing away what made it right.
    ///
    /// What it measures is narrower and more interesting than "does it know scales".
    /// The engine is handed the taught fingering by a rule of its own, so grading it
    /// with that rule switched on would measure the rule rather than the engine. It is
    /// graded with the rule off. The question is then whether the model of the hand,
    /// left to itself, arrives where a century of teaching arrived — and a setting that
    /// does that has better physics, not a better lookup table.
    Taught,
    ...
    pub fn apply_to_fingering(&self, options: &mut FingeringOptions) {
        for (name, value) in &self.0 {
            ...
            match name.as_str() {
                ...
                // `strain.*` is accepted and ignored, as any name the engine no longer
                // has is, so a weights file written before they were taken out of the
                // search still loads. See `knobs` for why they went.
                _ => {}
            }
        }
    }
...
fn knobs(target: Target) -> Vec<Knob> {
    let mut out = match target {
        ...
        Target::Fingers | Target::Play | Target::Taught => {
            ...
            knobs.extend([
                ...
                // `BiomechWeights::strain` is deliberately not here. The engine carries
                // those seven numbers but nothing reads them: the only two calls that
                // could, the inverse-kinematics solves in `BiomechModel`, hand `reach`
                // a `ReachRequest::new`, which fixes the strain weights at their
                // defaults. Searching them was searching seven directions that cannot
                // move the answer, and a search will happily spend its time there,
                // since every setting of them scores exactly alike.
                //
                // Plumbing them into `reach` is a real thing to want — it would let the
                // solver prefer a more comfortable posture rather than only judging the
                // one it finds. It is not free: postures would stop being a function of
                // the hand alone, and the shared cache in `biomech.rs` would have to be
                // keyed on them too, which for a search that changes them every
                // generation means solving every shape again every generation. That is
                // a decision about the engine, not about the search, and until it is
                // made these stay out.
            ]);
            knobs
        }
    };
    if target == Target::Taught {
        // Graded with the pattern rules off, so the weight on them cannot move the
        // answer — the same dead dimension the strain weights were, and left in it
        // would waste a fifth of every generation.
        out.retain(|knob| knob.name != "scale.pattern");
    }
...
/// A passage whose fingering is settled, and the fingers it is settled on.
pub struct Exercise {
    score: Score,
    /// The notes the convention has an opinion about, and what it says.
    want: Vec<(NoteId, Finger)>,
}

/// How many notes an exercise may run to, whatever its span and direction.
const EXERCISE_CEILING: usize = 64;

/// One exercise, decided entirely by `seed`.
///
/// Scales rather than arpeggios or broken chords, because the tables in `on-fingering`
/// cover scales and the chromatic and nothing else, and reference data invented to fill
/// the gap would teach the engine whatever was invented. Adding a family here is a
/// matter of adding its table there, from a source, and the generator will randomise
/// over it like the rest.
fn exercise(seed: u64) -> Exercise {
    let mut rng = seed | 1;
    let mut pick = |n: usize| (uniform(&mut rng) * n as f64) as usize % n.max(1);

    let hand = if pick(2) == 0 { Hand::Right } else { Hand::Left };
    // A chromatic scale one time in four. It is one fingering rather than twelve, so
    // more than that and the set is mostly the same exercise.
    let chromatic = pick(4) == 0;
    let tonic = pick(12) as u8;
    let octaves = 1 + pick(3);
    // Where on the keyboard, within the register each hand actually lives in.
    let floor = if hand == Hand::Right { 48u8 } else { 28u8 };
    let base = floor + 12 * pick(3) as u8 + tonic;
    let step = i64::from(TICKS_PER_QUARTER) / [1, 2, 2, 4][pick(4)];

    let mut up: Vec<u8> = Vec::new();
    if chromatic {
        up.extend((0..=(12 * octaves as u8)).map(|semitone| base + semitone));
    } else {
        const MAJOR: [u8; 7] = [0, 2, 4, 5, 7, 9, 11];
        for octave in 0..octaves as u8 {
            up.extend(MAJOR.map(|degree| base + degree + 12 * octave));
        }
        up.push(base + 12 * octaves as u8);
    }

    // Up, down, or up and back — the last being how a scale is practised, and the case
    // where the fingering has to turn around without the hand stopping.
    let mut pitches = match pick(4) {
        0 => up.iter().rev().copied().collect::<Vec<u8>>(),
        1 => up.clone(),
        _ => {
    ...
    pitches.truncate(EXERCISE_CEILING);
    ...
    // The answer comes from the engine's own published tables, which is the only place
    // in this repository that a taught fingering is written down. Asking them here and
    // then forbidding the solver to use them is the whole point: the tables say where a
    // century of teaching arrived, and the question is whether the hand model gets there
    // by itself.
    let heard: Vec<Option<u8>> = score.notes.iter().map(|n| Some(n.midi)).collect();
    let onsets: Vec<f64> = score.notes.iter().map(|n| n.onset_seconds).collect();
    let want = scale_fingerings(hand, &heard, &onsets)
        .into_iter()
        .map(|(index, taught)| (score.notes[index].id, taught.finger))
        .collect();
    Exercise { score, want }
}
...
pub enum Examples {
    ...
    /// Generated passages whose fingering is settled.
    Taught([Vec<Exercise>; 3]),
}

impl Examples {
    /// `count` generated exercises, divided three ways.
    ///
    /// They cost nothing to make and nobody had to annotate them, so there is no
    /// dataset to download and no licence to honour — and the held-out and test thirds
    /// are exercises the search genuinely never sees, since which third a seed falls in
    /// is fixed before any of them are built.
    pub fn taught(count: usize) -> Self {
        let mut parts: [Vec<Exercise>; 3] = Default::default();
        for seed in 0..count as u64 {
            let made = exercise(seed);
            // An exercise with nothing the convention has an opinion about teaches
            // nothing: a one-octave run is mostly its own ends, and the tables leave
            // those to the rest of the cost function.
            if made.want.is_empty() {
                continue;
            }
            parts[bucket(&format!("exercise-{seed}"))].push(made);
        }
        Examples::Taught(parts)
    }
    ...
    pub fn sizes(&self) -> [usize; 3] {
        match self {
            ...
            Examples::Taught(parts) => [parts[0].len(), parts[1].len(), parts[2].len()],
        }
    }
    ...
    fn name_of(&self, part: usize, index: usize) -> String {
        match self {
            ...
            Examples::Taught(parts) => {
                let exercise = &parts[part][index];
                let first = exercise.score.notes.first().map_or(0, |n| n.midi);
                format!("exercise from {first} of {} notes", exercise.score.notes.len())
            }
        }
    }
    ...
    pub fn target(&self) -> Target {
        match self {
            ...
            Examples::Taught(_) => Target::Taught,
        }
    }
        ...
        match self {
            ...
            Examples::Play(parts) => {
                ...
                for (_, score) in pick.iter().map(|i| &parts[part][*i]) {
                    ...
                    let faults = measured.stretched as f64
                        + UNPLAYABLE_PRICE * measured.unplayable as f64;
                    let chances = measured.spans_measured as f64;
                    good += (chances - faults).max(0.0);
                    of += chances;
                }
                (good, of)
            }
            Examples::Taught(parts) => {
                let mut options = fingering_options(weights);
                // The rule that hands the solver the taught fingering is switched off.
                // Left on, this measures whether that rule fired, which it does, and
                // every setting scores alike.
                options.pattern_scale = 0.0;
                let (mut right, mut judged) = (0.0, 0.0);
                for exercise in pick.iter().map(|i| &parts[part][*i]) {
                    let solution = finger_score(&exercise.score, &options);
                    for (note, want) in &exercise.want {
                        judged += 1.0;
                        right += f64::from(solution.finger_of(*note) == Some(*want));
                    }
                }
                (right, judged)
            }
...
/// What one transition a hand could not make costs, against one stretch beyond a
/// comfortable span.
///
/// Four. The two are not the same kind of fault — one is a fingering that cannot be
/// performed at all, the other one that can, tightly — but they are counted against the
/// same denominator, which is the number of hand shapes the music asked for. Every one
/// of those is a chance to be uncomfortable, and that is the thing being measured.
///
/// The denominator was the note count to begin with, and that was wrong in a way worth
/// recording. A fault can only happen where the hand takes a shape or moves between two,
/// and most notes are neither, so dividing by notes diluted the measure about five times
/// over: the engine's defaults read 99.55% across fifty thousand scores, leaving less
/// than half a point for a week of searching to find. Measured against the chances to go
/// wrong rather than against every note, the same engine has somewhere to go.
const UNPLAYABLE_PRICE: f64 = 4.0;
...
fn trim(score: &mut Score, notes: usize) {
    ...
    // note that is left.
    score.finalise();
    // Neither cutting nor finalising hands the memory back — a `Vec` that has been
    // shortened still holds the room it had at its longest. Fifty thousand scores cut
    // from eight hundred notes to a hundred and twenty were keeping 3.2 GB to hold
    // 450 MB of notes, which is the difference between this running on a Raspberry Pi
    // and not.
    score.notes.shrink_to_fit();
}
...
struct State {
    ...
    stalled: u64,
    /// Generations since the last setting was actually promoted, which is the only
    /// measure of progress that means anything.
    #[serde(default)]
    since_promotion: u64,
...
pub struct TuneConfig {
    ...
    pub threads: usize,
    /// Generations without a promotion before the target is called finished. 0 runs on
    /// regardless.
    pub give_up: u64,
    ...
    pub batch: usize,
    /// The other measure of the same weights, which a promotion may not make worse.
    ///
    /// The two fingering targets pull on the same knobs from opposite ends. Agreeing
    /// with a taught fingering buys thumb crossings that a measure of comfort alone
    /// would avoid, and nine minutes of searching one of them was enough to move the
    /// other nearly two points the wrong way. Run in turn without this they do not
    /// converge, they take turns undoing each other.
    ///
    /// So each is told what the other says, and may not spend it. A promotion has to
    /// improve the target being searched *and* leave the other where the engine's
    /// defaults had it, within the same slack a guarded piece gets.
    pub counterpart: Option<Examples>,
...
/// The smallest improvement worth calling one, as a fraction.
///
/// Five hundredths of a point. Without a floor here the only threshold is float
/// epsilon, and a search that tries eight hundred thousand settings against one held-out
/// third will always find something that beats it by a hair — not because the setting is
/// better but because that many draws from a coin will always turn up a run of heads.
///
/// That is not hypothetical. The hand target ran for 2.9 hours, tried 814,250 settings,
/// and promoted a gain of 0.004 points on the third it was allowed to see, while the
/// third it was not allowed to see went *down* by 0.013. Every one of those promotions
/// was the held-out set being fitted, one candidate at a time, and the log reported
/// progress the whole way.
///
/// Real gains here have been one to ten points. Nothing between zero and this floor has
/// ever turned out to be worth having.
const MIN_GAIN: f64 = 0.0005;

/// Generations without a promotion worth the name before a target is called finished.
///
/// It is counted in generations rather than in hours on purpose, because that makes it
/// scale with what a target costs: the hand target runs about twenty thousand
/// generations an hour and gives up after a quarter of one, the taught target runs
/// fifteen hundred and gets the best part of a working day. A target that has nothing
/// left should hand the machine to one that does, which is what `tune_forever` is for.
const GIVE_UP_AFTER: u64 = 5_000;
...
/// The default for [`TuneConfig::give_up`].
pub const DEFAULT_GIVE_UP: u64 = GIVE_UP_AFTER;
...
pub fn run(examples: &Examples, config: &TuneConfig, mut say: impl FnMut(&str)) -> Result<()> {
    // One setting per core is already as much parallelism as there is to have. The
    // solver splitting each score again, across the published rule sets and then across
    // the hands, only adds a hundred threads contending for twelve cores and a pair of
    // thread spawns per score to pay for it.
    ...
    // What the other measure says about the engine as it stands, which is the line a
    // promotion may not cross. Measured on one third of it: this is a gate, not
    // something to climb, and it is asked on every promotion.
    let counter_floor = config
        .counterpart
        .as_ref()
        .map(|other| other.score(&weights_of(&defaults), 0, threads));
    if let (Some(other), Some(floor)) = (&config.counterpart, counter_floor) {
        say(&format!(
            "Also watching the {} measure, at {:.2}%, which no promotion may spend.",
            other.target().name(),
            floor * 100.0
        ));
    }
    ...
    loop {
        ...
        if candidate_score > best_here + 1e-12 {
            ...
            state.evaluated += 2;
            // By a margin, not by a hair. A setting no better than the best wins a
            // straight comparison on an independent draw about half the time, so
            // without this the search confirms something nearly every generation, the
            // step grows to its ceiling and stays there, and it never settles down to
            // refine anything. That is what the hand target was doing for three hours:
            // step pinned at 0.95, stalled at 0, not one restart in sixty thousand
            // generations.
            confirmed = second[0] > second[1] + MIN_GAIN;
        }
        if confirmed {
        ...
        if confirmed {
            ...
            let better = held_out > state.best_scores.held_out + MIN_GAIN && scales_kept && sane;
            let kept = better
                && match (&config.counterpart, counter_floor) {
                    (Some(other), Some(floor)) => {
                        let now = other.score(&weights, 0, threads);
                        let held = now >= floor - GUARD_SLACK;
                        if !held {
                            crossed += 1;
                            event = "crossed";
                        }
            ...
            if kept {
                state.since_promotion = 0;
                state.best = candidate.clone();
                state.best_scores = Scores {
                    // The draw it was measured on, not the whole training third. The
                    // two figures below are the ones that mean anything — the held-out
                    // third decides the promotion and the test third is what gets
                    // reported — and both are measured over everything. This one is
                    // only ever printed, and the third it would be measured on is three
                    // fifths of the dataset: on the playing target that is four minutes
                    // of fingering, every promotion, for a number nobody acts on.
                    train: candidate_score,
                    held_out,
                ...
                event = "promoted";
                // Only the two figures measured over whole thirds are said aloud. The
                // learning one is now whatever this generation's draw made of it, which
                // is not a thing to put in front of somebody next to two that mean
                // something.
                say(&format!(
                    "Generation {}: better. {:.2}% held back, {:.2}% test \
                     (defaults {:.2}%). Saved to {}.",
                    state.generation,
                    held_out * 100.0,
        ...
        if config.give_up > 0 && state.since_promotion >= config.give_up {
            say(&format!(
                "Nothing better in {} generations. This target has run out of things to \
                 try at {:.2}% on the third it never learned from, against {:.2}% for the \
                 engine's defaults. Stopping so something else can have the machine; it \
                 carries on from here whenever it is started again.",
                state.since_promotion,
                state.best_scores.test * 100.0,
                state.baseline.test * 100.0
            ));
            break;
        ...
        if last_said.elapsed().as_secs() >= 600 {
            ...
            if guard_floor.is_some() {
                refused.push_str(&format!(
                    " {guarded} better settings refused for doing worse on a guarded piece."
                ));
            }
            if let Some(other) = &config.counterpart {
                refused.push_str(&format!(
                    " {crossed} refused for spending the {} measure.",
                    other.target().name()
                ));
            }
            say(&format!(
                "{:.2} hours searched, {} settings tried; \
                 best so far {:.2}% held back (defaults {:.2}%); \
                 {} generations since anything was worth keeping.{refused}",
                state.searched_seconds / 3600.0,
                state.evaluated,
...
mod tests {
    ...
    /// The harness has to be asking the engine a question it understands, or the
    /// figure it reports is about the harness rather than about the engine.
    ///
    /// Switch the pattern rules back on and the solver is handed the very table the
    /// exercises were labelled from, so it should agree with them nearly always. When
    /// this is low the exercises and the oracle have fallen out of step — a scale the
    /// generator built that the run finder does not recognise, an off-by-one in which
    /// note got which label — and the headline figure would be measuring that.
    #[test]
    fn the_engine_agrees_with_the_tables_when_it_is_allowed_to_see_them() {
        let examples = Examples::taught(150);
        let sizes = examples.sizes();
        assert!(sizes.iter().all(|n| *n > 0), "{sizes:?}");
        let Examples::Taught(parts) = &examples else { panic!("not the taught target") };

        let options = FingeringOptions::default();
        let (mut right, mut judged) = (0, 0);
        for exercise in parts.iter().flatten() {
            let solution = finger_score(&exercise.score, &options);
            for (note, want) in &exercise.want {
                judged += 1;
                right += usize::from(solution.finger_of(*note) == Some(*want));
            }
        }
        assert!(judged > 500, "only {judged} notes carried a taught fingering");
        let rate = right as f64 / judged as f64;
        assert!(rate > 0.95, "agreed on {:.1}% of {judged} taught notes", rate * 100.0);

        // And with them off it is a real question, which is the one the search is set.
        let bare = examples.score(&Weights::defaults(Target::Taught), 0, 4);
        assert!((0.3..0.95).contains(&bare), "nothing to learn here: {bare}");
    }

    /// A seed has to mean one exercise, or the thirds are not thirds: the same seed
    /// lands in the same third every run, and if it built something different each time
    /// the search would be tested on what it had trained on.
    #[test]
    fn a_seed_always_builds_the_same_exercise() {
        for seed in [1u64, 7, 99, 1234] {
            let (a, b) = (exercise(seed), exercise(seed));
            let pitches = |e: &Exercise| e.score.notes.iter().map(|n| n.midi).collect::<Vec<_>>();
            assert_eq!(pitches(&a), pitches(&b), "seed {seed}");
            assert_eq!(a.want.len(), b.want.len(), "seed {seed}");
        }
    }
