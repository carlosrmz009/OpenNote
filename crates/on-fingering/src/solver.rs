...
impl CostBreakdown {
    ...
    ///
    /// Summed over every chord of a hand's part this is the cost of that part, exactly —
...
impl Agreement {
    ...
    pub fn across_published(
        ...
        // The published sets are being asked what *they* think, so they are asked
        // without the agreement term — which does not exist yet — but with everything
        // else the real search uses.
        let ask = |set| {
...
/// Whether the solver may split its own work across threads of its own.
static INNER_THREADS: std::sync::atomic::AtomicBool = std::sync::atomic::AtomicBool::new(true);

/// Let the solver use threads of its own, or forbid it.
///
/// Fingering one score is four searches over the published rule sets, each of which is
/// two searches, one per hand — so a call spawns about ten threads and joins them. For
/// a single score, which is what Handy asks for, that is most of a modern machine put
/// to work and it stays on.
///
/// For anything that is *already* running one score per core it is the opposite of
/// help. `opennote tune` evaluates a dozen settings at once over thousands of scores:
/// with the inner threads on that is a hundred and twenty threads contending for twelve
/// cores, created and destroyed once per score, and the machine spends its time
/// switching rather than searching. Measured on the tuner, turning them off is worth
/// more than any other change made to it.
///
/// Process-wide, read once per call and never while a search is running, so it costs
/// nothing to leave on. The answer does not depend on it: the same work is done in the
/// same order and combined in the same order either way.
pub fn set_inner_threads(on: bool) {
    INNER_THREADS.store(on, std::sync::atomic::Ordering::Relaxed);
}

/// Whether the solver may spawn threads of its own right now.
...
fn solve_score(
    ...
    let search = |hand| {
        ...
        // The comfortable span, not the forced one. `max_prac` is what a hand can be
        // made to reach for an instant, with everything else it is doing subordinated
        // to getting there; it is not a shape a hand sits in while its other fingers go
        // on playing. Holding is the second thing, so it is measured by the second
        // number — and the two are far enough apart to matter, fourteen semitones
...
impl<'a> HandSolver<'a> {
    ...
    ///
    /// The posture term is charged relative to this, and it is worth being exact about
    /// what that does, because it is not what it looks like. **It changes no decision.**
    /// The same amount is taken from every candidate of one chord, so every path through
    /// that chord moves by the same amount and the cheapest path stays the cheapest; and
    /// since the baseline is the least strain there is, the `max(0.0)` it is paired with
    /// never binds either.
    ///
    /// What it changes is what the posture term *reads as*. Some chords are simply harder
    /// to hold than others, and that difficulty is not the fingering's doing. Measured
    /// from the easiest way of holding the same chord, the term says how much worse this
    /// particular shape is — which is the number worth putting in an explanation — rather
    /// than how hard the chord is, which would otherwise swamp every other term there.
    ...
    /// Everything the search charged that involves event `index`, with `here` put in
    /// place of whatever was chosen there and the rest of the path left as chosen.
    ///
    /// These are exactly the terms of the total that change when that one event changes:
    /// its own cost, the moves into and out of it, the three windows it sits in — its
    /// own and the one either side, which it is the neighbour of — and the three prior
    /// terms it touches, the one it is conditioned in and the two after it that it
    /// conditions. So the difference between two values of this is not an estimate of
    /// the difference in the total. It is that difference.
    fn neighbourhood(
        &self,
        events: &[Event],
        all: &[Vec<Candidate>],
        ...
        // The search asks the model nothing when a hand has a single chord, so neither
        // does this. See `single_event`.
        if events.len() >= 2 {
            for j in [i, i + 1, i + 2] {
    ...
    ///
    /// Each chord is given its share of the total the search compared, and the shares add
    /// up to that total exactly. A chord's own cost is its alone; a term the search
    /// charges between chords is given to one of them so it is counted once — the window
    /// to the chord at its centre, a move to the chord it arrives at, the prior to the
    /// chord it is a probability of. The margin is worked out over the whole
    /// [`neighbourhood`](Self::neighbourhood) instead, because asking what an
    /// alternative would have cost is asking about every term it would change.
    fn assemble(
        &self,
        events: &[Event],
        all: &[Vec<Candidate>],
        ...
        for (i, event) in events.iter().enumerate() {
            ...
            // worth flagging; a large one means it was forced. It is never negative:
            // the alternative is a path the search could have taken, and it took the
            // cheapest there was.
            let local =
...
/// The chord at position `j` of a hand's path, with `here` standing in at `index`.
///
/// `None` past either end, which is how the search leaves a window or a prior open at
/// the start and the end of a part.
fn on_path<'a>(
    events: &'a [Event],
    all: &'a [Vec<Candidate>],
    chosen: &[usize],
...
mod tests {
    ...
    /// both hands, and a run of two-note chords so that moves between shapes count.
    ...
    /// A small statistical model, so the prior term is actually charged. With the
    /// default options it is switched off, which is exactly how its absence from the
    /// explanations went unnoticed.
    ...
    /// The runs these tests are checked against: the engine as it ships, and the engine
    /// with a model loaded, since the second is the one that charges every term.
    ...
    /// A margin says how much the total would rise if this one note took a different
    /// finger and nothing else changed. That is another path the search could have taken,
    /// and it took the cheapest there was, so the margin can never be negative.
    ///
    /// It used to be. Changing one event changes three windows — the one centred on it
    /// and the one either side, which it is the neighbour of — and three prior terms,
    /// the one it is conditioned in and the two after it that it conditions. The margin
    /// counted one window and no prior at all, so it measured a different function from
    /// the one that decided, and `explain --close-calls` picked out notes by it.
    #[test]
    ...
    /// Each note's share of the cost has to add up to the total the search compared,
    /// or the breakdown describes some other function. The prior was missing from it
    /// entirely, which with a model loaded is the term most likely to have decided.
    ///
    /// Single lines only: a chord gives every one of its notes the same breakdown, so
    /// summing over notes would count it once per note.
    #[test]
